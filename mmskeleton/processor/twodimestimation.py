...
def batch_processor(models, datas, train_mode):
    ...
    outputs = dict(
        loss=loss, log_vars=log_vars, num_samples=datas[0].size(0))
...
def train(
    ...
    data_loaders = [torch.utils.data.DataLoader(
        dataset=call_obj(**d, transform = transforms.Compose([
            transforms.ToTensor(),
            normalize,
        ])),
...
def test(
    ...
    dataset = call_obj(**dataset_cfg, transform = transforms.Compose([
            transforms.ToTensor(),
            normalize,
        ]))
    ...
    all_preds = np.zeros(
        (num_samples, model_cfg.skeleton_head.num_joints, 3),
        dtype=np.float32
    )
    ...
    with torch.no_grad():
        for i, (input, meta, target, target_weight) in enumerate(data_loader):
            ...
            preds, maxvals = get_final_preds(
                test_cfg.post_process, output.detach().cpu().numpy(), c, s)
        ...
        name_values, perf_indicator = dataset.evaluate(
                test_cfg, all_preds, work_dir, all_boxes, image_path,
                filenames, imgnums
            )
...
def inference(
    ...
    detection_model = init_detector(detection_model_file,
                          ...
                          device='cuda:0')
    from IPython import embed; embed()
    detection_model = MMDataParallel(detection_model, device_ids=range(gpus)).cuda()
    ...
    for idx, image in enumerate(data_loader):
        skeleton_resluts = inference_model(image, detection_model, skeleton_model)
