...
class DemoRealtime(IO):
    ...
    def start(self):
        ...
        while(True):
            ...
            # model predict
            voting_label_name, latest_frame_label_name, output, intensity = self.predict(
                data)

            # visualization
            app_fps = 1 / (time.time() - tic)
            image = self.render(data_numpy, voting_label_name,
                                latest_frame_label_name, intensity, orig_image, app_fps)
    ...
    def predict(self, data):
        ...
        return voting_label_name, latest_frame_label_name, output, intensity

    def render(self, data_numpy, voting_label_name, latest_frame_label_name, intensity, orig_image, fps=0):
        images = utils.visualization.stgcn_visualize(
            data_numpy[:, [-1]],
            self.model.graph.edge,
            intensity[[-1]], [orig_image],
            voting_label_name,
            [latest_frame_label_name],
            self.arg.height,
            fps=fps)
        image = next(images)
